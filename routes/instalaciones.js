...
app.get('/getPiso', async (req, res) => {
  ...
  db.any(`SELECT instalacion.id_instalacion,instalacion.nombre,tipoinstalacion.nombre as tipoinstalacion,sector.nombre as sector,campus.nombre as campus, instalacion.descripcion, instalacion.piso,instalacion.latitud,instalacion.longitud,instalacion.foto
    ...
    .catch((err) => {
      return res.status(500).json({
        ok: false,
        mensaje: "Error No se encuentra el piso seleccionado",
        errors: err,
      });
    });
})
...
//http://18.231.149.121:3000/instalacion/filtro?nombre=206&id_campus=1&id_tipo=1
app.get('/filtro', async (req, res) => {
  ...
  db.any(`SELECT ins.id_instalacion,ins.nombre,tipo.nombre as tipoinstalacion,sec.acronimo as sector,cam.acronimo as campus, ins.descripcion, ins.piso,ins.latitud,ins.longitud,ins.foto
            FROM instalacion as ins 
            INNER JOIN sector as sec ON ins.id_sector = sec.id_sector 
            INNER JOIN campus as cam ON sec.id_campus = cam.id_campus
            INNER JOIN tipoinstalacion as tipo ON ins.id_tipo = tipo.id_tipo
            WHERE ins.nombre = $1 AND ins.id_tipo = $2 AND sec.id_campus = $3;`, [nombre, id_tipo, id_campus])
    .then((instalaciones) => {
      res.status(200).json({
        ok: true,
        instalaciones: instalaciones,
